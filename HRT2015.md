...
In the case of blue edges (consisting of a red vertex into a green vertex) in one st-node of the st-decomposition, we need to store additional information to check whether there exists a red-to-green path in the same node. We assign at most 2 numbers to each vertex, such that if the red vertex $u$ remembers $i,j \in \mathbb{N}$ and green vertex $v$ remembers $l,r \in \mathbb{N}$, then $u$ reaches $v$ iff $i \leq l \leq j$, $i \leq r \leq j$, min{l,r}$\leq j < i or $j < i \leq$ max{l,r}.
