...
Let $L_i(v), \hat{L_i}(v), R_i(v), \hat{R_i}(v)$ be totally ordered by its starting vertex position on the disegments of $F_x$. Now we want to compute these two best vertices in an efficient way. Represent each function $l_i(v), r_i(v)$ by a rooted forest with appropriate parent pointers and use a level-ancestor structure on that forest to answer queries quickly.

For vertices $v \in V$ we create rooted trees $T_l, T_r$ with parent pointers $p_l, p_r$ to define functions that will allow us to compute these two 'best' vertices easily. We define $p_l[v] = l_{d_2[v]-1}(v)$ if $d_2[v] \neq 0$, else $\perp$. Similarly, $p_r[v] = r_{d_2[v]-1}(v)$ if $d_2[v] \neq 0$, else $\perp$. In other words, these parent pointers give us the best vertex in the corresponding partition from the previous level.

Next define the functions $l'_i(v), r'_i(v)$ as **nearest ancestors to v** in these rooted trees $T_l, T_r$ respectively, that have depth at most $i$.
...
What this lemma says is: if the nearest ancestor of $l_{i+1}(v)$ on the left partition of vertices at depth $i$ is not the best vertex to $v$ at depth $i$, then there must exist a crossing vertex $m \in \{ r_{i+1}(v), l_{i+1}(v) \}$ such that the left and right parents of $m$ are the best vertices that can reach $v$ on level $i$. Intuitively, this is saying that a crossing must happen if the parent at depth $i$ of the best vertex at level $i+1$ is worse (on depth i) than the best vertex at level $i$ itself.
